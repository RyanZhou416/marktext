<template>
  <div class="key-input-dialog">
    ...
    <AppDialog
      v-model:open="showKeyInputDialog"
      width="350px"
      @update:open="
        val => {
          if (!val) cancelKeybinding()
        }
      "
    >
...
export default {
  ...
  methods: {
    ...
    handleKeyDown(event) {
      ...
      this.isKeybindingValid =
        keybinding.isValid && isValidElectronAccelerator(keybinding.accelerator)
...
<style>
.key-input-dialog :deep(.app-dialog-content) {
  box-shadow: none !important;
  border: none !important;
  background: none !important;
}
.key-input-dialog :deep(.app-dialog-body) {
  padding: 0 !important;
  margin: 0 !important;
}
</style>
